...
def sanitize_coordinates_np(_x1, _x2, img_size, padding=0):
    """
    Sanitizes the input coordinates so that x1 < x2, x1 != x2, x1 >= 0, and x2 <= image_size.
    Also converts from relative to absolute coordinates and casts the results to long tensors.
...
def crop_np(masks, boxes, padding:int=1):
    """
    "Crop" predicted masks by zeroing out everything not in the predicted bbox.
    Vectorized by Chong (thanks Chong).
    ...
    """
    h, w, n = masks.shape
    x1, x2 = sanitize_coordinates_np(boxes[:, 0], boxes[:, 2], w, padding)
    y1, y2 = sanitize_coordinates_np(boxes[:, 1], boxes[:, 3], h, padding)

    rows = np.tile(np.arange(w, dtype=x1.dtype).reshape(1, -1, 1), [h, 1, n])
    cols = np.tile(np.arange(h, dtype=x1.dtype).reshape(-1, 1, 1), [1, w, n])
